#!/bin/bash
#
# File to normalize m log files. Remove computer-specific,
...
do
	...
	cat $1 | python -c "import sys,re;sys.stdout.write(re.sub('\r?\nstartup_exists = exist.+[\r\n]+if startup_exists.+[\r\n]+else[\r\n]+.+clear startup_exists[\r\n]+','',sys.stdin.read()))" > $1.temp1
	sed -bf $(which normalize_m_log.sed) $f.temp1 > $f
	rm -f $f.temp1
done
